...
#define NUM_LEDS 300 //change this for the number of LEDs in the strip
#define COLOR_ORDER RGB
CRGB leds[NUM_LEDS];

extern const TProgmemRGBPalette16 ChaseBlueColors_p FL_PROGMEM =
{
  CRGB::MidnightBlue,
  CRGB::DarkBlue,
  CRGB::LightBlue,
...
extern const TProgmemRGBPalette16 CandyCane_p FL_PROGMEM =
{
  CRGB::Red,
  CRGB::Black,
  CRGB::GhostWhite,
...
const uint8_t paletteMultiplier = 2;
...
CRGBPalette16 gLastPalette( CRGB::Black );
...
void loop() {
  ...
  do {
    ...
    if( data == 255 ) {
      ...
      gCurrentPaletteNumber = data;
    }
    //Serial.println(data);
    gTargetPalette = gGradientPalettes[ gCurrentPaletteNumber ];


    Serial.print("Current Pallet: ");
    Serial.println(gCurrentPaletteNumber);
    //nblendPaletteTowardPalette(gCurrentPalette,gTargetPalette,16);
    fill_solid(leds,NUM_LEDS,CRGB::Black);
    uint8_t offset = 0;
    for( int count=0; count < (NUM_LEDS*2); count++ ) {
      for( int led=0; led < NUM_LEDS; led++ ) {
        //Serial.println(offset);
        leds[led] = ColorFromPalette( gTargetPalette, ((led+offset) * paletteMultiplier) % 256, 150, LINEARBLEND);
        //RAMP_UP(led, led+20);
      }
      offset++;
      FastLED.show();
...
void Random(int startpos,int endpos) {
  if( endpos >=  NUM_LEDS) {
    endpos = NUM_LEDS-1;
  }
  for( int pos = startpos; pos < endpos; pos++ ){
    leds[pos] = CRGB(random(0,150),random(0,150),random(0,150));
  }
  FastLED.show();
  delay(10);
}

void RAMP_UP(int startpos,int endpos) {
  //Serial.println(startpos);
  if( endpos >=  NUM_LEDS) {
    endpos = NUM_LEDS-1;
  }
  fill_gradient(leds,startpos, CHSV(160,200,200), endpos, CHSV(0,0,255),FORWARD_HUES);
  /*
  for(int i=0; i < 10; i++) {
    int led_pos = pos+i;
    if( led_pos >=  NUM_LEDS) {
    ...
    leds[led_pos] = CRGB(0+(i*20),0+(i*20),255-(i*20));
  }*/
  //FastLED.show();
}
